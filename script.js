...
const courseDetailsButton = document.querySelector("button.show-course-details");
const projectDetailsButton = document.querySelector("button.show-project-details");
let td1, td2, comparison;

// choose 2 techdegrees to compare
const selectTD = function() {
  if (td1 === undefined) {
    td1 = techdegreeData[chooseButtons.indexOf(this)];
    this.removeEventListener("click", selectTD);
    this.classList.add("selected");
  } else if (td2 === undefined) {
    td2 = techdegreeData[chooseButtons.indexOf(this)];
    this.classList.add("selected");
...
// reformat td data as arrays of objects for each course/project
const formatData = (td, courseProject) => {
  let holderArray = [];
  comparison[td][courseProject].forEach(item => {
    if (comparison[td][courseProject].indexOf(item) % 2 === 0) {
      holderObject = {
        techdegree: [`${comparison[td].name}`],
        name: item,
      };
    } else {
      holderObject['details'] = item;
      holderArray.push(holderObject);
    };
  });
  comparison[td][courseProject] = holderArray;
};
...
const findCommon = (courseProject) => {
  ...
  comparison['td2'][courseProject].forEach(item => {
    ...
    found ? holderArray[index].techdegree.push(comparison['td2']['name']) : holderArray.push(item);
  });
  comparison[courseProject] = holderArray;
};

//  ----- compare the techdegrees - prepares all the data
const compareCourses = () => {
  formatData('td1', 'courses');
  formatData('td1', 'projects');
  formatData('td2', 'courses');
  formatData('td2', 'projects');
  findCommon('courses');
  findCommon('projects');
};

// --- update HTML to show course/project data
const updateHTML = (courseProject) => {
  let Td1 = document.querySelector(`section.${courseProject}>ul.td-1`);
  let Td12 = document.querySelector(`section.${courseProject}>ul.td-1-2`);
  let Td2 = document.querySelector(`section.${courseProject}>ul.td-2`);
  Td1.childNodes[1].childNodes[1].innerText = `${comparison['td1']['name']}`;
  Td12.childNodes[1].childNodes[1].innerText = `${comparison['td1']['name']} & ${comparison['td2']['name']}`;
  Td2.childNodes[1].childNodes[1].innerText = `${comparison['td2']['name']}`;
  comparison[courseProject].forEach(item => {
    let html = `<li><p class="${courseProject}-name">%name%</p><p class="${courseProject}-details hide">%details%</p></li>`;
    html = html.replace("%name%", item.name).replace("%details%", item.details);
    item.techdegree.length > 1 ? Td12.insertAdjacentHTML('beforeend', html) : item.techdegree == comparison['td1']['name'] ? Td1.insertAdjacentHTML('beforeend', html) : Td2.insertAdjacentHTML('beforeend', html);
  });
};

// reset HTML to remove course/project data\
const resetHTML = (courseProject) => {
  let Td1 = document.querySelector(`section.${courseProject}>ul.td-1`);
  let Td12 = document.querySelector(`section.${courseProject}>ul.td-1-2`);
  let Td2 = document.querySelector(`section.${courseProject}>ul.td-2`);
  let listArray = [Td1, Td12, Td2];
  listArray.forEach(item => {
    item.childNodes[1].childNodes[1].innerText = "%name%";
    while (item.childNodes.length > 3) {
        item.removeChild(item.lastChild);
    };
  });
};

// explore button: run the comparison and display in the UI
...
const reset = () => {
  ...
  chooseButtons.forEach(item => {
    item.addEventListener("click", selectTD);
    item.classList.remove("selected", "disabled", "transparent");
  });
  td1 = td2 = comparison = undefined;
  main.classList.add("hide");
  header.scrollIntoView({behavior: "smooth", block: "start"});
};

// show details (course type or project description)
const showDetails = (courseProject) => {
  (document.querySelectorAll(`.${courseProject}-details`)).forEach(item => item.classList.toggle("hide"));
};
...
(() => {
  ...
  courseDetailsButton.addEventListener("click", function() {showDetails('courses')});
  projectDetailsButton.addEventListener("click", function() {showDetails('projects')});
})();
